...
def update_plot_scan(file2,file3,file4,file5,file6,file7,file8,file9,file,point1,point2,graph_config,theme_switch):
    ...
    if config['o2_measurement_switch']['on']:
        ...
        if config['o2_autosave_switch']['on']:
            f = open(o2_file[0],'w')
            f.write(o2_file[1])
            f.close()
...
def get_parameters(file):
    # get parameter data
    if "-scan" in file: # for multi-scan measurements, there is only one parameter file.
        new_file = file[:file.index('-scan')] + '.csv'
        print(new_file)
        try:
            get_parameters(new_file) # check for edge case, where '-scan' is part of the name of a single-scan measurement
            file = new_file
        except:
            print("no multi-scan parameter file found")
            pass
