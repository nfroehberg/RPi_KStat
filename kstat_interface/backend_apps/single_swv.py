...
# Single Square Wave Voltammetry Measurement Procedure
# Nico Fröhberg, 2020
...
def single_swv(config, motor, ser):
    # disable controls
...
def swv_measurement(config, motor, ser, file):
    ...
    frequency=config['frequency_input']['value']
    ...
    scan_time = (abs(start_potential-end_potential)/step_size)/frequency
    
    print('Start SWV Measurement')
    ...
    s_measurement.enter(purge_time,2,KStat.squarewaveVoltammetry,(
        ...
        pulse_height,frequency,n_scans,samplefreq,file,comment,True))
