...
def single_cv(config, motor, ser):
    write_config([{'component':'purge_switch','attribute':'disabled','value':True},
    ...
    if purge_time > 0:
        ...
        for i in range(purge_time*4+1):
            s_auxiliary.enter(i/4,1,make_progress,(i,purge_time*4)) # progress bar
    ...
    if cleaning_time > 0:
        ...
        for i in range(cleaning_time*4+1):
            s_auxiliary.enter(purge_time+(i/4),2,make_progress,(i,cleaning_time*4)) # progress bar
    ...
    if deposition_time > 0:
        ...
        for i in range(deposition_time*4+1):
            s_auxiliary.enter(purge_time+cleaning_time+(i/4),2,make_progress,(i,deposition_time*4)) # progress bar
    ...
    for i in range(int(scan_time)*4+1):
            s_auxiliary.enter(purge_time+cleaning_time+deposition_time+(i/4),2,make_progress,(i,scan_time*4)) # progress bar
